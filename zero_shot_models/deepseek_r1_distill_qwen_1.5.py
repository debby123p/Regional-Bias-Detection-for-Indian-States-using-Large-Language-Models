...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Zero-shot binary classification for regional bias detection using DeepSeek models')
    ...
    parser.add_argument('--offload-dir', type=str, default=None,
                        help='Directory for model offloading') # Specify offload directory if using CPU offloading
...
def setup_environment(args):
    ...
    # Create output directories - ensure parent directory exists first
...
# Function removed as it was redundant
...
"""
        ...
        with torch.no_grad():
            try:
                ...
                # Fallback mode with restricted parameters
                logger.info("Retrying with more restricted parameters")
...
def process_dataset(classifier, df, output_dir, save_interval=1, max_comment_length=500):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing comments"):
        ...
        if (idx + 1) % save_interval == 0 or idx == len(df) - 1:
            ...
            logger.info(f"Saved progress after comment {idx+1}")
        
        # Clear memory after each comment
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            torch.cuda.synchronize()
        gc.collect()
...
def find_latest_checkpoint(output_dir):
    ...
    # Extract chunk numbers and find latest
    chunk_nums = []
    ...
    logger.info(f"Found checkpoint: {latest_file} (chunk {latest_chunk_num})")
...
def main():
    ...
    for chunk_start in range(0, total_rows, chunk_size):
        ...
        try:
            ...
            # Continue with smaller chunk size
            continue
...
if __name__ == "__main__":
    ...
    # Run classifier
    logger.info("Starting regional bias classification...")
    main()

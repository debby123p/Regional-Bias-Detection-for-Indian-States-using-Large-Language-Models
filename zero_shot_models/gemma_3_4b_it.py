...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Zero-shot binary classification for regional bias detection using Gemma models with memory optimization')
    
    # Data and output paths
    parser.add_argument('--data-path', type=str, required=True,
                        help='Path to the annotated dataset CSV file')
    parser.add_argument('--output-dir', type=str, default='results/gemma/',
...
def setup_environment(args):
    ...
    log_file = os.path.join(args.output_dir, f"gemma_classification_{timestamp}.log")
...
class RegionalBiasClassifier:
    ...
    def classify_with_cot(self, text, max_length=500):
        ...
        cot_prompt = f"""Determine if this comment contains regional bias about Indian states or regions:
...
"""
    ...
    def _parse_response(self, response, full_response):
        ...
        try:
            ...
            classification = "non_regional_bias"  # Default
            ...
            if "classification:" in lower_response:
                # Split by the classification marker
                parts = lower_response.split("classification:", 1)
                if len(parts) > 1:
                    class_text = parts[1].strip()
                    # Extract just the first word/phrase after "Classification:"
            ...
            # If no explicit classification, look for keywords in the full text
            ...
            # Try to extract reasoning separately from classification
            if "classification:" in lower_response:
                reasoning = lower_response.split("classification:")[0].strip()
            
            # If we still don't have a clear decision, check for explicit mentions
...
def save_results(results, output_dir, timestamp, prefix="gemma"):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing comments"):
        ...
        if (idx + 1) % save_interval == 0 or idx == len(df) - 1:
            ...
            intermediary_file = os.path.join(
                intermediary_dir, 
                f"gemma_comment_{idx+1}_of_{len(df)}_{timestamp}.csv"
            )
...
if __name__ == "__main__":
    ...
    # Run classifier
    logger.info("Starting regional bias classification with Gemma model...")
    main()

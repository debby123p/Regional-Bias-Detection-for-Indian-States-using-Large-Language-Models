...
"""
Few-shot learning for regional bias detection using Qwen2.5-7B-Instruct with a full-context approach.
This implementation places all 200 examples in the prompt context to leverage Qwen's powerful 
instruction-following capabilities for classification decisions.
"""

def parse_arguments():
    """Parse command line arguments"""
    parser = argparse.ArgumentParser(description='Few-shot learning for regional bias detection with Qwen2.5')
...
def create_qwen_prompt(examples_df, comment, random_seed=42):
    """
    Create a prompt for few-shot learning with Qwen's instruction format
...
def setup_model(model_name, cache_dir, gpu_id, hf_token):
    """
    Load model and tokenizer with Qwen-specific settings
...
def predict_with_qwen(model, tokenizer, prompt, device, max_tokens=10):
    """
    Generate prediction using Qwen model with optimized parameters
    ...
    # For Qwen, we need to be careful with the response extraction
    generated_text = ""
    try:
        # Try to extract the part after "Classification:"
        if "Classification:" in full_output:
            generated_text = full_output.split("Classification:")[-1].strip()
        else:
            # Fallback to last 50 characters
    ...
    # If we don't have a clear number, look more thoroughly at the text
...
def batch_predict(model, tokenizer, test_df, examples_df, device, random_seed=42):
    """
    Process comments in batches for inference with Qwen-specific optimizations
    ...
    # Process examples individually - Qwen needs specific handling
    for i in range(0, len(test_comments)):
        comment = test_comments[i]
        
        # Generate prompt with all examples
        prompt = create_qwen_prompt(examples_df, comment, random_seed)
        ...
        if input_ids_length > 8000:  # If too long, use a simplified prompt
            ...
            # Simplified prompt for long inputs - Qwen format
            prompt = "<|im_start|>system\nYou are an expert in identifying regional biases in comments about Indian states and regions.\n<|im_end|>\n"
            prompt += "<|im_start|>user\nTask: Classify if the comment contains regional bias related to Indian states or regions.\n\n"
            prompt += "Instructions:\n"
            prompt += "- Regional Bias (1): Comments that contain stereotypes, prejudices, or biases about specific Indian states or regions.\n"
        ...
        # Add a brief pause before prediction
        if i > 0 and i % 5 == 0:
            print("Pausing briefly to optimize model performance...")
            time.sleep(1)  # 1-second pause every 5 examples
        
        # Get prediction
        prediction, raw_output = predict_with_qwen(model, tokenizer, prompt, device)
        ...
        # Clear cache to prevent OOM errors - Qwen needs frequent clearing
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            gc.collect()
...
def save_results(test_df, predictions, raw_outputs, output_dir, model_name="qwen_7b_200examples"):
    """

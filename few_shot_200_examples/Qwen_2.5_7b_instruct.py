...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{LOG_DIR}/qwen2.5_7b_200examples_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
MODEL_NAME = "qwen2.5_7b_instruct_200examples"
...
def create_few_shot_prompt(examples_df, comment):
    ...
    # Create the prompt - Keeping EXACTLY the same format as in the original
...
def load_model():
    """Load Qwen model and tokenizer with optimized settings for larger context window"""
    ...
    # Configure to use the maximum available GPU memory with improved context window

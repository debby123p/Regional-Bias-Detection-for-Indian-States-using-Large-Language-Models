...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{LOG_DIR}/mistral_7b_200examples_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
# Model name for saving files - updated for Mistral
MODEL_NAME = "mistral_7b_instruct_v0.3_200examples"
...
def load_model():
    """Load Mistral-7B-Instruct-v0.3 model and tokenizer with optimized settings for larger context window"""
    logger.info("Loading mistralai/Mistral-7B-Instruct-v0.3 model...")
    start_time = time.time()
    
    # Changed model ID to Mistral-7B-Instruct-v0.3
    ...
    # Configure tokenizer for Mistral

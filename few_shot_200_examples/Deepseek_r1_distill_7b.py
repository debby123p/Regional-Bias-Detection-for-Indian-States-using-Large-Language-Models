...
"""
Few-shot learning for regional bias detection using a full-context approach with 200 examples.
This implementation places all examples in the prompt context to leverage maximum information
for classification decisions.
"""

def parse_arguments():
    """Parse command line arguments"""
    parser = argparse.ArgumentParser(description='Few-shot learning for regional bias detection with 200 examples')
...
def create_few_shot_prompt(examples_df, comment, random_seed=42):
    """
    Create a prompt for few-shot learning with all 200 examples and the target comment.
...
def batch_predict(model, tokenizer, test_df, examples_df, device, random_seed=42):
    ...
    for i in range(0, len(test_comments)):
        ...
        if torch.cuda.is_available() and (i + 1) % 3 == 0:  # More frequent clearing for 7B model
            torch.cuda.empty_cache()
            gc.collect()
    
    return predictions, raw_outputs

def save_results(test_df, predictions, raw_outputs, output_dir, model_name="deepseek_7b_200examples"):
    """

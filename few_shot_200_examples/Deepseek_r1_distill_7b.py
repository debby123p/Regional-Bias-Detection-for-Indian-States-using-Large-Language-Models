import os
# Select to use only GPU 0
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # Using GPU 0 directly
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{LOG_DIR}/deepseek_r1_distill_qwen_7b_200examples_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
# Set up directories - update to use the 200-example dataset
EXAMPLE_PATH = "/DATA2/akash/venvs/debasmita/data/balanced_dataset_200_comments.csv"
TEST_PATH = "/DATA2/akash/venvs/debasmita/data/annotated_experiment_phase_new - Sheet1.csv"
OUTPUT_DIR = "/DATA2/akash/venvs/debasmita/results/few_shot_200"

# Ensure output directory exists
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Model name for saving files - updated for DeepSeek 7B
MODEL_NAME = "deepseek_r1_distill_qwen_7b_200examples"

# Configure for GPU 0 directly
torch.cuda.empty_cache()
torch.cuda.set_device(0)  # Directly set to GPU 0
...
def load_datasets():
    ...
    # Load few-shot examples - now using 200 examples (100 each class)
...
def create_few_shot_prompt(examples_df, comment):
    ...
    Uses all 200 examples from the example dataset.
    Keep the original prompt format exactly as is.
    
    Args:
        examples_df: DataFrame with 200 example comments
    ...
    bias_examples = examples_df[examples_df[class_column] >= 1] if class_column == 'Level-1' else examples_df[examples_df[class_column] == 1]
    non_bias_examples = examples_df[examples_df[class_column] == 0]
    ...
    # Create the prompt - Keeping EXACTLY the same format as specified
...
def load_model():
    """Load DeepSeek-R1-Distill-Qwen-7B model and tokenizer with optimized settings for larger context window"""
    logger.info("Loading deepseek-ai/DeepSeek-R1-Distill-Qwen-7B model...")
    start_time = time.time()
    
    # Changed model ID to DeepSeek-R1-Distill-Qwen-7B
    ...
    # Configure tokenizer for DeepSeek
    tokenizer = AutoTokenizer.from_pretrained(
        model_id, 
        token=hf_token,
        cache_dir=CACHE_DIR,
        trust_remote_code=True,  # May be needed for DeepSeek models
        model_max_length=8192,   # Increase context window for better understanding
    )
    ...
    # For 7B model with 200 examples, use 4-bit quantization for better memory efficiency
    quantization_config = BitsAndBytesConfig(
        load_in_4bit=True,           # 4-bit quantization for more memory efficiency
        bnb_4bit_use_double_quant=True,
        bnb_4bit_compute_dtype=torch.float16
    )
    
    # Load with optimized parameters for GPU 0
    model = AutoModelForCausalLM.from_pretrained(
        model_id,
        token=hf_token,
        cache_dir=CACHE_DIR,
        quantization_config=quantization_config,
        device_map="cuda:0",  # Using GPU 0 directly
...
def predict_with_model(model, tokenizer, prompt, max_tokens=10):
    ...
    try:
        ...
        # Using a larger max_length to accommodate more examples
        inputs = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=8192).to(device)
        
        # Add a slight delay before generation to improve quality
        time.sleep(0.5)  # Half-second delay for better quality
        ...
        # Add a slight delay after generation
        time.sleep(0.5)  # Half-second delay for better quality
...
def batch_predict(model, tokenizer, test_df, examples_df, batch_size=1):
    ...
    # Process examples individually with better pacing
    for i in range(0, len(test_comments)):
        ...
        try:
            ...
            if input_ids_length > 8000:  # Increased from 4000 to handle larger context
            ...
            # Add a brief pause before prediction for better quality
            if i > 0 and i % 10 == 0:
                logger.info("Pausing briefly to optimize model performance...")
                time.sleep(2)  # 2-second pause every 10 examples
        ...
        # Add a small delay between examples for better quality
        time.sleep(0.5)  # Half-second pause between examples
...
def main():
    ...
    if bias_count != 100 or non_bias_count != 100:
        logger.warning(f"Expected 100 examples of each class, but found {bias_count} bias and {non_bias_count} non-bias examples")
    ...
    logger.info("Using improved context window and processing for higher quality results.")

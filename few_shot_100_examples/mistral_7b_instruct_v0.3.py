...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{LOG_DIR}/deepseek_7b_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
def create_few_shot_prompt(examples_df, comment):
    ...
    # Create the prompt - Mistral style
...
def load_model():
    """Load Mistral model and tokenizer with optimized settings"""
    ...
    # Load with optimized parameters for 40GB GPU
...
def predict_with_model(model, tokenizer, prompt, device="cuda", max_tokens=10):
    """

...
def create_few_shot_prompt(examples_df, comment, random_seed=42):
    ...
    Returns:
        Formatted prompt string optimized for Mistral's chat format
    """
    ...
    # Create the prompt with Mistral's chat format
...
def predict_with_model(model, tokenizer, prompt, device, max_length=4096, max_tokens=10, logger=None):
    """
    Generate prediction using model with Mistral-specific response extraction
    ...
    # Generate response with optimized parameters for Mistral
    with torch.no_grad():
    ...
    # Extract model's response (after the instruction marker)
    ...
    # Look for explicit "1" or "0" in the response
    ...
    # If no explicit number at beginning or end, check within the text
    if "1" in generated_text and not "0" in generated_text:
        return 1, full_output
    elif "0" in generated_text and not "1" in generated_text:
        return 0, full_output
    
    # Look for bias-related keywords
    if "regional bias" in generated_text.lower() or "bias" in generated_text.lower():
        return 1, full_output

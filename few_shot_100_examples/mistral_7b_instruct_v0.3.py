...
DEFAULT_MAX_LENGTH = 4096  # Smaller context window for fewer examples

def parse_arguments():
    """Parse command line arguments with sensible defaults"""
    parser = argparse.ArgumentParser(description='Few-shot learning for regional bias detection with Mistral (100 examples)')
    ...
    parser.add_argument('--output_dir', type=str, 
                        default=os.environ.get('OUTPUT_DIR', 'results/mistral_few_shot_100'),
...
def load_datasets(examples_path, test_path, logger, test_limit=None):
    ...
    # Load few-shot examples - this contains our 100 examples (50 per class)
    ...
    # Check if we have the expected number (50 of each)
    expected_count = 50
...
def create_few_shot_prompt(examples_df, comment, random_seed=42):
    ...
    # Create the prompt - Mistral style with chat format
    prompt = "<s>[INST] Task: Classify if the comment contains regional bias related to Indian states or regions.\n\n"
    prompt += "Instructions:\n"
...
def setup_model(model_name, cache_dir, gpu_id, hf_token, logger):
    ...
    try:
        ...
        # Configure quantization - using 8-bit for 100 examples scenario
        ...
        # Load model - Mistral specific settings
        model_kwargs = {
...
def predict_with_model(model, tokenizer, prompt, device, max_length=4096, max_tokens=10, logger=None):
    ...
    try:
        ...
        # Generate response - Mistral specific generation parameters
        with torch.no_grad():
            outputs = model.generate(
                **inputs,
                ...
                num_beams=1,      # Simple greedy decoding for Mistral with 100 examples
        ...
        # Extract just the model's response (after our prompt) - Mistral specific
        ...
        # Check for number at the end too
        if generated_text.endswith("1") or generated_text == "1":
            return 1, full_output
        elif generated_text.endswith("0") or generated_text == "0":
            return 0, full_output
        
        # If we don't have a clear number, look more thoroughly at the entire response
    ...
    for i in range(0, len(test_comments)):
        ...
        try:
            ...
            if output_dir and ((i + 1) % checkpoint_interval == 0 or i == len(test_comments) - 1):
                ...
                model_short_name = "mistral_7b_100examples"
...
def save_results(test_df, predictions, raw_outputs, output_dir, logger, model_name="mistral_7b_100examples"):
    """

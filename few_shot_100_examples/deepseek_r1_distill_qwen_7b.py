import os
# Select to use only GPU 1 (the second GPU)
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # This makes GPU 1 appear as device 0 to PyTorch
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{LOG_DIR}/deepseek_r1_distill_qwen_7b_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
OUTPUT_DIR = "/DATA2/akash/venvs/debasmita/results/few_shot_100"
...
torch.cuda.set_device(0)  # Since we've hidden other GPUs, device 0 now refers to the physical GPU 1
...
def create_few_shot_prompt(examples_df, comment):
    ...
    # Create the prompt - optimized for DeepSeek-R1 reasoning abilities
...
def load_model():
    """Load DeepSeek-R1-Distill-Qwen-7B model and tokenizer"""
    logger.info("Loading DeepSeek-R1-Distill-Qwen-7B model...")
    ...
    # Load with optimized parameters for GPU 1 (80GB A100)
    model = AutoModelForCausalLM.from_pretrained(
        model_id,
        token=hf_token,
        cache_dir=CACHE_DIR,
        quantization_config=quantization_config,
        device_map="cuda:0",  # Now this refers to physical GPU 1 due to CUDA_VISIBLE_DEVICES
...
def predict_with_model(model, tokenizer, prompt, max_tokens=50):
    """
    ...
    try:
        ...
        # Generate response - more tokens for reasoning models
        with torch.no_grad():
            outputs = model.generate(
                **inputs,
                max_new_tokens=max_tokens,  # Increased for reasoning
        ...
        if full_output.startswith(prompt):
            ...
            generated_text = full_output[-100:].strip()  # Get more text for reasoning models
        ...
        # If we don't have a clear number, look for keywords
        ...
        # More advanced classification for reasoning models
    ...
    for i in range(0, len(test_comments)):
        ...
        try:
            ...
            if input_ids_length > 4000:
                ...
                # If too long, use a simplified prompt
                prompt = "You are an expert in identifying regional biases in comments about Indian states and regions. "
                prompt += "Task: Classify if the comment contains regional bias related to Indian states or regions.\n\n"
                prompt += "Instructions:\n"
                prompt += "- Regional Bias (1): Comments that contain stereotypes, prejudices, or biases about specific Indian states or regions.\n"

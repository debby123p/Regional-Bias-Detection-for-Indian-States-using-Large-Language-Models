...
def load_datasets(examples_path, test_path, logger, num_examples=150, test_limit=None, random_seed=42):
    ...
    Args:
        ...
        num_examples: Number of few-shot examples to use (default: 150)
    ...
    if len(examples_df) > num_examples:
        ...
        logger.info(f"Selected balanced subset of {len(selected_examples)} examples for few-shot learning (targeting {num_examples} examples)")
    else:
...
"""
    ...
    # For 150 examples approach, we'll include more examples but with less detailed format
    # This keeps the prompt from becoming too long
...
def predict_with_model(model, tokenizer, prompt, device, max_length=4096, max_tokens=50, logger=None):
    ...
    """
    try:
        # Check if prompt is too long and warn
        tokenized_prompt = tokenizer.encode(prompt)
        if len(tokenized_prompt) > max_length:
            if logger:
                logger.warning(f"Prompt is very long: {len(tokenized_prompt)} tokens, which exceeds max_length of {max_length}. Truncation will occur.")
...
def main():
    ...
    # Display HuggingFace token status
    if args.hf_token:

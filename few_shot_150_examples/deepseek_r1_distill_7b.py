import os
# Select which GPU to use (device 1)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{LOG_DIR}/deepseek_r1_distill_qwen_7b_150examples_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
MODEL_NAME = "deepseek_r1_distill_qwen_7b_150examples"
...
def create_few_shot_prompt(examples_df, comment):
    ...
    # Create the prompt - DeepSeek style
...
def load_model():
    """Load DeepSeek-R1-Distill-Qwen-7B model and tokenizer with optimized settings"""
...
def predict_with_model(model, tokenizer, prompt, max_tokens=10):
    ...
    try:
        ...
        # Handle potential missing token IDs (common issue when switching models)

...
def setup_model(model_name, cache_dir, gpu_id, hf_token, logger):
    ...
    try:
        # Configure tokenizer - DeepSeek has specific tokenizer settings
        tokenizer_kwargs = {
            'trust_remote_code': True,  # Might be needed for DeepSeek models
        }
        ...
        # Load model - DeepSeek has specific loading requirements
        model_kwargs = {
...
def predict_with_model(model, tokenizer, prompt, device, max_length=4096, max_tokens=10, logger=None):
    ...
    try:
        ...
        # Generate response - DeepSeek generation parameters
        with torch.no_grad():
            outputs = model.generate(
                **inputs,
                ...
                num_beams=1,      # Simple greedy decoding for DeepSeek with 150 examples
        ...
        # Look for explicit "1" or "0" at the beginning or in the first few characters
        # (DeepSeek tends to put the answer at the start)
